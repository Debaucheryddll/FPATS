...
class RemoteParameter:
    ...
    # 使用python装饰器方法，使得当你在读取value的时候优先返回本地缓存的值
    # 当本地没有值的时候才会真正通过网络向服务器请求
    # 而当你修改value值的时候，它会立即发送给服务器，从而更新FPGA中的状态
    @property
    def value(self) -> Any:
    ...
    def add_callback(self, callback: Callable, call_immediately: bool = False):
        ...
        # 针对你感兴趣的参数注册一个当其值发生变化时需要被自动调用的函数
...
class RemoteParameters:
    def __init__(self, remote: LinienControlService, uuid: str, use_cache: bool):
        ...
        """
        self.remote = remote           # 确定对端socket
        self.uuid = uuid               # 确定本次连接客户端的唯一的uuid，用于服务器区别哪个客户端在跟它说话
