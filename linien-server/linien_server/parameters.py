...
class Parameters:
    ...
    def __init__(self):
       ...
       volts peak-peak, e.g: `parameters.sine_source_am_amplitude.value = 0.2 * Vpp`.
        ...
        self.tpfc_kalman_proc_noise_t = Parameter(start=1e-1, min_=0, max_=1, restorable=True)
        """Process noise (time dimension)."""

        self.tpfc_kalman_proc_noise_f = Parameter(start=1e-1, min_=0, max_=10, restorable=True)
        """Process noise (frequency dimension)."""

        self.tpfc_kalman_proc_noise_drift = Parameter(start=1e-2, min_=0, max_=1, restorable=True)
        """Process noise (frequency drift dimension)."""

        self.tpfc_kalman_meas_noise_base = Parameter(start=1e-2, min_=0, max_=1.0, restorable=True)
        """Base measurement noise std dev (seconds)."""

        self.tpfc_kalman_meas_noise_fade = Parameter(start=1e-1, min_=0, max_=1.0, restorable=True)
        ...
        self.tpfc_kalman_init_cov_t = Parameter(start=10.0, min_=0, max_=1e6, restorable=True)
        """Initial covariance for time offset."""

        self.tpfc_kalman_init_cov_f = Parameter(start=10.0, min_=0, max_=1e6, restorable=True)
        """Initial covariance for frequency offset."""

        self.tpfc_kalman_init_cov_d = Parameter(start=10.0, min_=0, max_=1e6, restorable=True)
        ...
        self.scan_lock_uncertainty = Parameter(start=500.0, min_=0, max_=5000.0, restorable=True)
        self.scan_power_threshold = Parameter(start=2000, min_=0, max_=65535, restorable=True)
        self.scan_narrow_timeout = Parameter(start=1.0, min_=0.01, max_=10.0, restorable=True)
