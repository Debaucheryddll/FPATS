...
class PitayaAnalog(Module):
    def __init__(self, adc, dac):
        # adc和dac都是实例，使用一种依赖注入的方式被传递给PitayaAnalog类
        # adc和dac对象的定义和创建发生在更高层次的平台文件或顶层设计文件中，在本项目中对应hw_playform.py
        self.comb += adc.cdcs.eq(1), adc.clk.eq(0b10)
        # 配置adc芯片的cdcs和clk为固定值

        # sign = 1<<(len(dac.data) - 1)
        size = len(dac.data), True
        # a，b分别表示两路信号，adc为输入，dac为输出
        self.adc_a = Signal(size)
        self.adc_b = Signal(size)
        self.dac_a = Signal(size)
        self.dac_b = Signal(size)

        adca = Signal.like(adc.data_a)##like作用为复制一个已有信号，生成一个属性相似的新信号
        adcb = Signal.like(adc.data_b)
        # self.sync表示同步逻辑，在有效时钟沿变化时执行的电路，一下代码是为了将adc两路信号的时序进行同步
        # 使ADC原始数据变为一个与FPGA的sys时钟同步的稳定版本
        ...
        self.comb += [
            ...
            #进行格式转换的代码是从adca的第二位开始取的，第0位和第1位被直接丢弃了，这与hw平台中的定义相对应
            self.adc_a.eq(Cat(~adca[2:-1], adca[-1])),
            self.adc_b.eq(Cat(~adcb[2:-1], adcb[-1])),
        ]
        # comb中的代码完成的是数据格式的转化工作
