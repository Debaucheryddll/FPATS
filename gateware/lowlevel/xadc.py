...
# 该部分代码作用与analog中使用的高速、外部DAC不同
# 这段代码的全部内容都是为了驱动和读取Xilinx FPGA内部自带的XADC硬核，不需要过多关注
# XADC 是Xilinx公司在很多FPGA芯片内部集成的一个片上系统监控模块。它是一个多通道的ADC
# 但它的主要设计用途不是处理高速的外部信号，而是监控FPGA芯片自身的状态。
from migen import (
    Case,
    Cat,
    ClockSignal,
